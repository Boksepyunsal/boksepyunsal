...
API Gateway V2 앞단에 Cognito Authorizer를 두어 인증을 처리하고, AWS KMS + crypto-js로 민감 데이터(계량값, 청구 정보)를 암호화해 엔터프라이즈 보안 요건을 충족했습니다. 프론트엔드는 React + TypeScript + Redux Toolkit으로 구성하고, Google Maps API로 전국 설비 위치를 지도에 시각화하며 Recharts로 실시간 발전량 트렌드를 제공합니다.`,
...

  {
    ...
    client: 'GS (대기업)',
    role: '프론트엔드 개발 · AWS 데이터 파이프라인',
    period: '2023.01 – 2023.08',
    desc: 'AWS Athena + Step Functions 기반 수억 건 에너지 시계열 데이터 분석·예측 플랫폼. 기존 수십 분이던 쿼리를 수 초로 단축하고, ECharts로 실시간 발전량 트렌드를 시각화.',
    ...
    overview: '산업용 열에너지 공급망의 수요를 예측하고 최적화하는 Heat Operation System입니다. AWS Athena를 쿼리 엔진으로 도입해 수억 건의 시계열 에너지 데이터를 서버리스로 실시간 분석하고, Step Functions로 예측 파이프라인을 완전 자동화했습니다.',
...
프론트엔드는 React + Vite + TailwindCSS로 빠른 개발·빌드 속도를 확보하고, ECharts를 활용해 수천 개 포인트의 시계열 데이터를 부드럽게 렌더링했습니다. 대시보드, 트렌드 분석, 최적화 시뮬레이션, 단가 관리 4개 주요 모듈로 구성했습니다.`,
...

  {
    ...
    client: 'GS (기업 내부용)',
    role: 'AWS 아키텍처 · AI 엔지니어링',
    period: '2024.01 – 2024.06',
    desc: 'AWS Bedrock(Claude 3.5 Sonnet) + OpenSearch 기반 엔터프라이즈 RAG 파이프라인. Lambda + DynamoDB 서버리스 백엔드를 CDK IaC로 배포하고, LangGraph 에이전트가 사내 지식베이스를 하이브리드 검색. 관리자 대시보드에서 AI 성능을 실시간 모니터링.',
    tags: ['AWS Bedrock', 'Claude 3.5 Sonnet', 'LangGraph', 'OpenSearch', 'RAG', 'AWS CDK', 'DynamoDB', 'Python'],
    ...
    overview: '수천 건의 사내 문서·정책·규정을 자연어로 검색하고 출처와 함께 답변하는 생성형 AI 챗봇입니다. AWS Bedrock Claude 3.5 Sonnet과 OpenSearch Serverless를 결합한 RAG 파이프라인을 AWS CDK IaC로 배포하고, LangGraph 에이전트가 하이브리드 검색으로 정확도를 높입니다.',

    stats: [
      { label: '프로젝트 규모', value: '6개월' },
      { label: '담당 역할', value: 'AWS · AI 엔지니어링' },
...
답변에는 출처 문서와 페이지를 인용해 할루시네이션 리스크를 최소화했습니다. Bedrock Titan Embeddings로 임베딩을 생성하고, DynamoDB로 대화 이력을 저장해 멀티턴 컨텍스트를 유지합니다. 전체 인프라는 AWS CDK IaC로 코드화해 재현 가능한 배포를 보장합니다. React 기반 관리자 대시보드에서 쿼리 수·정확도·응답 시간을 실시간 모니터링하고, Human-in-the-loop 피드백(👍/👎)으로 AI 품질을 지속 개선합니다.`,
    ...
    techStack: {
      ...
      Infra: ['AWS CDK (IaC)', 'Serverless Framework', 'GitHub Actions'],
    },
    ...
    features: [
      ...
      'AWS CDK IaC — 전체 인프라 코드화 및 재현 가능 배포',
    ],
...

  {
    id: 5,
    slug: 'baro',
    name: 'FitB — Terraform IaC · AWS 인프라 구축',
    client: 'PurpleMoon Project Group',
    role: '인프라 구축 · Terraform IaC · 풀스택',
    period: '2023.04 – 2023.10',
    desc: 'Terraform으로 AWS 인프라를 코드화하고 GitHub Actions CI/CD로 자동 배포. Docker 컨테이너 기반 Spring Boot + GraphQL 서버를 EC2에 프로비저닝. App Store·Google Play 출시까지 완료한 헬스케어 앱 인프라 구축.',
    tags: ['Terraform', 'AWS EC2', 'Docker', 'GitHub Actions', 'Spring Boot', 'GraphQL', 'Flutter', 'PostgreSQL'],
    gradient: 'linear-gradient(135deg, #0ea5e9 0%, #06b6d4 100%)',
    icon: `<svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
      <rect x="2" y="3" width="20" height="14" rx="2"/>
      <path d="M8 21h8M12 17v4"/>
      <path d="M7 8h2M7 11h5M7 14h8"/>
    </svg>`,
    size: 'medium',
    ...
    overview: 'Terraform IaC로 AWS 인프라를 코드화하고, Docker + GitHub Actions로 Spring Boot GraphQL 서버를 자동 배포하는 DevOps 파이프라인을 구축했습니다. Flutter BLoC 기반 헬스케어 앱 FitB의 전체 인프라를 담당해 App Store·Google Play 양 스토어 출시를 완료했습니다.',

    stats: [
      { label: '프로젝트 규모', value: '6개월' },
      { label: '담당 역할', value: '인프라 · Terraform' },
      { label: '출시 플랫폼', value: 'App Store + Play' },
      { label: '인프라', value: 'IaC (Terraform)' },
    ],

    problem: `사이드 프로젝트 특성상 인프라를 빠르게 구축하면서도 팀원 누구나 동일한 환경을 재현할 수 있어야 했습니다. 수동 서버 설정은 환경 불일치 문제를 유발하고, 코드 변경 시 서버에 직접 접속해 배포하는 과정이 번거로웠습니다.

앱 스토어 출시를 위한 백엔드 API의 안정적인 운영과, 모바일 앱의 오버패칭 문제를 해결할 효율적인 API 설계도 필요했습니다.`,

    solution: `Terraform으로 AWS VPC, EC2, RDS(PostgreSQL), Security Group 등 전체 인프라를 코드화해 누구나 동일한 환경을 재현할 수 있도록 했습니다. Docker로 Spring Boot 애플리케이션을 컨테이너화하고, GitHub Actions로 코드 푸시 시 자동 빌드·테스트·EC2 배포까지 CI/CD 파이프라인을 구성했습니다.

백엔드는 Java 17 + Spring Boot 3 + GraphQL로 타입 안전한 API를 설계해 모바일의 오버패칭 문제를 해결했고, Flutter BLoC 패턴으로 앱 상태 관리를 명확히 분리했습니다.`,

    techStack: {
      Infra: ['Terraform', 'AWS EC2', 'AWS RDS (PostgreSQL)', 'AWS VPC', 'Docker'],
      'CI/CD': ['GitHub Actions', 'Docker Hub', 'SSH Deploy'],
      Backend: ['Java 17', 'Spring Boot 3.3', 'GraphQL', 'JPA', 'Spring Security OAuth2'],
      'Mobile App': ['Flutter', 'Dart', 'BLoC 패턴', 'graphql_flutter'],
    },

    features: [
      'Terraform IaC — VPC · EC2 · RDS · Security Group 전체 코드화',
      'GitHub Actions CI/CD — 코드 푸시 → 빌드 → 테스트 → EC2 자동 배포',
      'Docker 컨테이너 기반 Spring Boot 서버 운영',
      'GraphQL API — 오버패칭 없는 모바일 최적화 데이터 통신',
      'Google / Apple 소셜 로그인 (Spring Security OAuth2)',
      'App Store · Google Play 양 스토어 출시 완료',
    ],

    outcome: `Terraform IaC로 인프라 환경 재현 시간을 수 시간에서 수 분으로 단축했습니다. GitHub Actions CI/CD 도입으로 수동 배포 작업을 완전 자동화해 팀 생산성을 높이고, Docker 컨테이너화로 로컬과 운영 환경의 일관성을 보장했습니다. App Store와 Google Play 양쪽 스토어 출시에 성공했습니다.`,
  },
